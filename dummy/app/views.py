...
try:
    ...
    # COLEÇÕES DE COMPRADOR E ESTOQUE REMOVIDAS CONFORME SOLICITADO
...
def register_user(request):
    if request.method == 'POST':
        try:
            ...
            user_data = {
                "name": name, "profile_picture": "", "email": email, "password": password,
                "phone": phone, "zip_code": zip_code, "cpf": cpf, "points": 0, "discarded_items": 0
            }
    ...
    if request.method == 'POST' and 'user_id' in request.session:
        ...
        try:
            updated_data = {
                ...
                "cpf": request.POST.get('cpf'), "phone": request.POST.get('phone'),
                "zip_code": request.POST.get('zip_code'), "password": int(request.POST.get('password'))
            }
...
def partner_register(request):
    if request.method == 'POST':
        try:
            data = {
                "NomeParceiro": request.POST.get('NomeParceiro'), "CpfParceiro": request.POST.get('CpfParceiro'),
                "EmailParceiro": request.POST.get('EmailParceiro'), "CepParceiro": request.POST.get('CepParceiro'),
                "LogradouroParceiro": request.POST.get('LogradouroParceiro'), "ComplementoParceiro": request.POST.get('ComplementoParceiro'),
                "SenhaParceiro": int(request.POST.get('SenhaParceiro')), "PontosParceiro": 0, "FotoPerfilParceiro": ""
            }
...
def manage_collection_points(request):
    ...
    try:
        ...
        context = {'points': points_list, 'partner_cpf': partner_cpf}
...
def create_collection_point(request):
    ...
    if request.method == 'POST':
        try:
            ...
            data = {
                "CnpjParceiro": request.POST.get('CnpjParceiro'), "ItensColetados": 0, "Pontuacao": 0,
                "CpfParceiro": partner_cpf, "EnderecoPonto": request.POST.get('EnderecoPonto'),
...
def update_collection_point(request, point_id):
    if request.method == 'POST' and 'partner_id' in request.session:
        try:
            updated_data = {
                "CnpjParceiro": request.POST.get('CnpjParceiro'), "EnderecoPonto": request.POST.get('EnderecoPonto'),
                "HorarioAbertura": request.POST.get('HorarioAbertura'), "CapacidadeKg": int(request.POST.get('CapacidadeKg')),
                "Pontuacao": int(request.POST.get('Pontuacao', 0)), "ItensColetados": int(request.POST.get('ItensColetados', 0)),
            }
...
def register_item(request):
    ...
    if request.method == 'POST':
        try:
            ...
            data_item = {
                "Valor": float(request.POST.get('Valor')), "ModeloItem": request.POST.get('ModeloItem'),
                "Tipo": request.POST.get('Tipo'), "IdPontoColeta": ObjectId(id_ponto_coleta),
                "IdEstoque": None, "DataRegistro": datetime.now()
            }
    ...
    context = {'points': available_points}
...
def item_inventory(request):
    ...
    try:
        ...
        points_cursor = collection_point_collection.find({"CpfParceiro": partner_cpf})
        point_ids = [point['_id'] for point in points_cursor]

        inventory_items = list(item_collection.find({"IdPontoColeta": {"$in": point_ids}}))

        point_map = {point['_id']: point['EnderecoPonto'] for point in collection_point_collection.find({"_id": {"$in": point_ids}})}
        
        for item in inventory_items:
            item['EnderecoPonto'] = point_map.get(item['IdPontoColeta'], 'Ponto Deletado')

        context = {'items': inventory_items}
        return render(request, 'item_inventory.html', context)
        
    except Exception as e:
        messages.error(request, f"Erro ao carregar inventário: {e}")
        return redirect('partner_area')
    ...
    if request.method == 'POST' and 'partner_id' in request.session:
        try:
            ...
            if not item:
                ...
                return redirect('item_inventory')
                
            point_id = item.get('IdPontoColeta')
            ...
            messages.success(request, "Item removido do inventário com sucesso.")
            
        except Exception as e:
            messages.error(request, f"Erro ao deletar item: {e}")
